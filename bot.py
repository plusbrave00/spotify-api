...
# ==================== ADB HELPERS ====================
async def run_adb_command(command, user_id=None):
    """Run an ADB command with timeout."""
    try:
        device_ip = get_device_ip(user_id) if user_id else None
        if device_ip and "-s" not in command:
            full_cmd = f"adb -s {device_ip} {command}"
        else:
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        f"👑 *Admin Only:*\n"
        f"/genkey <days> - Generate promo key\n"
        f"/approve <request_id> - Approve request\n"
        f"/reject <request_id> - Reject request{BRANDING}"
    )
...
async def screen_snapshot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result = await run_adb_command(f"-s {device_ip} shell screencap -p /sdcard/screenshot.png", user_id)

    if "Error" in result or "timed out" in result:
        await update.message.reply_text(f"❌ Screenshot failed: {result}{BRANDING}")
        return
    ...
    try:
        pull_result = await run_adb_command(f"-s {device_ip} pull /sdcard/screenshot.png {temp_file}", user_id)
        ...
        if os.path.exists(temp_file) and os.path.getsize(temp_file) > 0:
            ...
            await update.message.reply_text(f"❌ Failed to capture screenshot{BRANDING}")
    finally:
        if os.path.exists(temp_file):
            os.remove(temp_file)
        if os.path.exists(temp_dir):
            os.rmdir(temp_dir)
        await run_adb_command(f"-s {device_ip} shell rm /sdcard/screenshot.png", user_id)
...
async def battery_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result = await run_adb_command(f"-s {device_ip} shell dumpsys battery", user_id)

    if "Error" in result or "timed out" in result:
        await update.message.reply_text(f"❌ Battery check failed: {result}{BRANDING}")
        return
...
async def setdevice_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            ...
            f"Example: /setdevice 192.168.1.100:5555{BRANDING}",
            parse_mode=ParseMode.MARKDOWN,
        )
        return
...
async def shell_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            ...
            f"Example: /shell pm list packages{BRANDING}",
            parse_mode=ParseMode.MARKDOWN,
        )
        return

    command = " ".join(context.args)
    await update.message.reply_text(f"⏳ Running: `{command}`", parse_mode=ParseMode.MARKDOWN)

    result = await run_adb_command(f"-s {device_ip} shell {command}", user_id)
...
async def check_adb_services(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result = await run_adb_command("start-server")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📱 Status & Device":
        ...
        update_device_ip(user_id, text)
        await update.message.reply_text(
            f"✅ *Device Connected!*\n\n"
            f"🔗 *Device:* `{text}`{BRANDING}",
            parse_mode=ParseMode.MARKDOWN,
        )
